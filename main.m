...
%   title={A simulation tool to interpret error rates in LoRa systems under frequency-sweeping jamming},
...
% How many times will we run the simulation for a given signal-to-interference ratio (SIR)?
% The output variable (symbol error rate - SER) will be averaged based on
% this quantity
...
% Jamming sweep time, in seconds
SP = 5e-6;          

% LoRa transmitting frequency
f_channel = 868.0e6;                                    
...
for k = 1:runs
    ...
    [lora_bb,jam_bb,noise_bb,fs_lora_upsamp,B_lora,a,Tsym,M,Pjam_10MHz] = signal_generator(A_lora,A_jam,SP,f_channel);
